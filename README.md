...
We are **exposing port 80** so open your browser to <http://localhost:8080/> and once the video is completed you will see the link with the video.
If you attach to the container and watch the logs you can also wait to see the "Visualization process is complete" message printed.

## Source Types

Single remote git repo url.

```shell
    -e GIT_URL=https://github.com/kubernetes/kubernetes.git
```

Single volume mounted local git repo.

```shell
    -v /path/to/your/repo:/visualization/git_repo:ro
```

Multiple volume mounted local git repos. The mounted dir should contain multiple repo folders for which each will have a custom log generated which is then combined.

```shell
    -v /path/to/your/repos:/visualization/git_repos:ro
```

Single volume mounted local git log. If you have generated a custom formated log file.
...
| -------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `GIT_URL`                  | `<docker repo on GH>`            | URL of git repository to be cloned and analyzed for visualization. (optional)                               |
